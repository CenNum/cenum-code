...
const COMMANDS = [
  ...
  { name: "/setting", description: "查看/修改设置", hint: "[askBeforeWrite|askBeforeBash] [on|off]" },
];
  ...
  useInput((input, key) => {
    ...
    if (key.return) {
      if (ss && mc.length > 0) {
        ...
        setCursorPos(mc[idx].name.length + 1);
        setSuggestionIdx(0);
      } else {
    ...
    if (key.backspace || key.delete) {
      setValue(v => {
        if (cursorPosRef.current > 0) {
          return v.slice(0, cursorPosRef.current - 1) + v.slice(cursorPosRef.current);
        }
        return v;
      });
      setCursorPos(p => Math.max(0, p - 1));
    ...
    if (input && input.length > 0) {
      ...
      if (filtered.length > 0) {
        setValue(v => {
          ...
          return v.slice(0, pos) + filtered + v.slice(pos);
        });
        setCursorPos(p => p + filtered.length);
        setShowCursor(true);
      }
    }
  });
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {confirmMode ? null : (() => {
        const prefix = "> ";
        // 在 cursorPos 处插入光标符号
        const valWithCursor = value.slice(0, cursorPos) + cursor + value.slice(cursorPos);
        const full = prefix + valWithCursor;
        const termWidth = process.stdout.columns || 80;
        const lines: string[] = [];
        for (let i = 0; i < full.length; i += termWidth) {
          lines.push(full.slice(i, i + termWidth));
        }
        return (
          <Box flexDirection="column">
            {lines.map((line, i) => {
              const isPrompt = i === 0 && line.startsWith(prefix);
              return (
                <Box key={i}>
                  <Text color={isPrompt ? "cyan" : undefined} bold={isPrompt}>
                    {line}
                  </Text>
                </Box>
              );
